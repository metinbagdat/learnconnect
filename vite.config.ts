  ...
  build: {
    ...
    rollupOptions: {
      ...
      output: {
        ...
        // ✅ CRITICAL FIX: Simple object-based chunk configuration
        // Ensures React loads first and prevents "React is undefined" errors
        // Explicit ordering prevents cross-chunk dependency issues
        manualChunks: {
          // React MUST load first - explicit bundle, guaranteed load order
          'react-core': ['react', 'react-dom', 'react/jsx-runtime'],
          // Everything else stays in index bundle to prevent initialization race conditions
          // This eliminates the generic 'vendor' chunk that was loading before React
        },
